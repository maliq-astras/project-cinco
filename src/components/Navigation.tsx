          ...
          <div className={navigationStyles.navBar}>
            ...
            <nav className={navigationStyles.nav}>
              <div className={navigationStyles.dropdownContainer} ref={dropdownRef}>
                ...
                {isDropdownOpen && (
                  <div 
                    className={navigationStyles.dropdownMenuClass}
                    style={navigationStyles.dropdownMenu(colors.primary)}
                  >
                    {menuItems.map((item, index) => (
                      <button
                        key={index}
                        onClick={item.onClick}
                        className={`${inter.className} ${navigationStyles.menuItemClass}`}
                        style={navigationStyles.menuItem(colors.primary)}
                      >
                        <span>{item.label}</span>
                        {item.showArrow && (
                          <svg 
                            className={navigationStyles.menuItemArrow} 
                            fill="none" 
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                          >
