# 🔴 RED-005: First Card Animation Refresh Race Condition
...
2. **Page refresh during 1000ms animation delay** - State gets persisted mid-animation:
   ...
   - `revealedFacts: [0]` (✅ persisted after 1000ms delay)
   - Timer state corrupted (❌ won't reinitialize)
...
3. **Game reload with inconsistent state**:
   ...
   - Sets `canMakeGuess: false` (❌ incorrectly)
   - Sets `canRevealNewClue: true` (❌ should be false - user should guess first)
...
closeFactCard() {
  ...
  if (!isTimerActive && !hasSeenClue) {
    resetTimer();
    startTimer(); // ❌ This condition becomes false after hasSeenClue=true
  }
}
...
if (hasHydrated && !isTimerActive && !gameState.isGameOver && !persistedHasSeenClue) {
  resetTimer(); // ❌ Skipped when hasSeenClue=true from persisted state
}
```
...
- ✅ **First card only** (subsequent cards work fine)
- ✅ **During card flip animation** (1000ms window)
- ✅ **Page refresh timing** (must hit the async delay window)
- ✅ **Animation persists** (fact gets added to revealed stack)
